...
window.onload = () => {
    ...
    if (vistaComprobante) {
        showToast("La Nota y Compra han sido guardadaos satisfactoriamente.");
        var historial = window.history;
        historial.pushState({}, '', "Compra.html?id_empresa="+empresaNow+"&currentUser="+userNow);
...
function mostrarModalNota(seleccion) {
    ...
    tablaDetalles.innerHTML=
        ...
        GetBDLotes_X_IdNota(datos=>{
            const bodyTable=tablaDetalles.getElementsByTagName("tbody")[0];
            bodyTable.innerHTML='';
            let TotalDetalle=0;
            datos.result.lote.forEach(element => {
                bodyTable.innerHTML=bodyTable.innerHTML+
                    '<tr>'+
                        '<td>'+element.nroLote+'</td>'+
                        '<td>'+element.nombre+'</td>'+
                        '<td>'+invertirFecha(element.fechaVencimiento)+'</td>'+
                        '<td>'+element.cantidad+'</td>'+
                        '<td style="text-align:right;">'+element.precioCompra+'</td>'+
                        '<td style="text-align:right;">'+parseFloat(element.cantidad)*parseFloat(element.precioCompra)+'</td>'+
                    '</tr>';
                TotalDetalle=TotalDetalle+(parseFloat(element.cantidad)*parseFloat(element.precioCompra));
            });
...
document.getElementById("btnAnularNota").addEventListener("click",function (e) {
    GetBDLotes_X_IdNota(data=>{
        let findVenta= false;
        data.result.lote.forEach(item => {
            if(item.cantidad>item.stock && findVenta ==false){
                findVenta=true;
                showToast(45);
        ...
        if(findVenta == false){
            AnularNotaLogic(notaDetected, 1);
        }else{
            document.getElementById("cerrarAnular").click();
        }
    },notaDetected)
    //;
});
...
function AgregarCompra(){
    newDirigirXnombre("AddCompra");
}

function imprimirComprobante() {
    const iFrameInformes = document.getElementById("iFrameInformeCompra");
    document.getElementsByClassName("modal-view")[0].style.display="none";
    ...
    iFrameInformes.setAttribute("src", "http://localhost/ReportsERP/report/Empresas_ERP/ReporteNotaCompra?Parameter_IdNota=" + notaDetected + "&rs:Embed=true");
